<!DOCTYPE html>
<html lang="vi"

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Dịch vụ Shopee - Uy tín - Chuyên nghiệp - An toàn</title>
    <meta http-equiv="Cache-Control" content="no-cache">
    <meta http-equiv="Expires" content="-1">
    <meta name="keywords" content="dich vu shopee, shopee , dịch vụ shopee">
    <meta name="description" content="Cung cấp dịch vụ bán hàng hiệu quả trên Shopee">
    <meta property="og:url" content="http://www.dichvushopee.info" />
    <meta property="og:title" content="Dịch vụ shopee" />
    <meta property="og:type" content="website" />
    <meta property="og:image" content="/img/logo_tb.png">
    <meta property="og:description" content="Cung cấp dịch vụ bán hàng hiệu quả trên Shopee" />
    <meta name="format-detection" content="telephone=no" />
    <meta name="msvalidate.01" content="A6D0C0B7EE858196E4E1D854C2D7AE06" />

    <link rel="preload" href="{{ asset('v2/source/ladipage.min.js?v=1588161697899') }}" as="script">
    <link rel="icon" type="image/png" sizes="192x192"  href="/android-icon-192x192.png">
    <link rel="icon" type="image/png" sizes="96x96" href="/favicon-96x96.png">
    <link rel="apple-touch-icon" sizes="180x180" href="/apple-icon-180x180.png">
    <meta name="msapplication-TileColor" content="#ffffff">
    <meta name="msapplication-TileImage" content="/ms-icon-144x144.png">
    <meta name="theme-color" content="#ffffff">
    @include('frontend.css')

    <style>
        @media (min-width: 768px) {
            #FORM_ITEM281 {
                width: 441.724px;
                height: 150.207px;
                top: 30.379px;
                left: -300px;
            }
            #FORM_ITEM1997 {
                width: 441.724px;
                height: 150.207px;
                top: 30.379px;
                right: -300px;
            }

            #FORM_ITEM2002 {
                width: 441.724px;
                height: 150.207px;
                top: 220.379px;
                right: 0;
            }
            .thrv_wrapper {
                width: 56px;
                height: 56px;
                border-radius: 50%;
                overflow: hidden;
                z-index: 3;
                position: relative;
                margin: 0px auto !important;
                top: 32.255px;
                left: -175.6px;
            }
            .tve_image_caption {
                padding: 0;
                max-width: 100% !important;
                -webkit-box-sizing: border-box !important;
                box-sizing: border-box !important;
            }
            .tve_image_caption .tve_image_frame {
                display: block;
                max-width: 100%;
                position: relative;
                overflow: hidden;
            }
            .scrollRegister {
                top: 200.65px !important;
                left: 200px !important;
            }
            #HEADLINE125 {
                width: 420px;
                top: 315px;
                left: 0px;
            }
            #HEADLINE125 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE126 {
                width: 433px;
                top: 378px;
                left: 0px;
            }
            #HEADLINE126 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE127 {
                width: 420px;
                top: 435px;
                left: 0px;
            }
            #HEADLINE127 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #LINE148 {
                height: 490px;
                top: 8px;
                left: 618px;
            }
            #LINE148 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE148 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48);
                border-bottom: 2px solid rgb(232, 104, 48);
                border-left: 2px solid rgb(232, 104, 48);
            }
            #LINE149 {
                width: 619px;
                top: 366.504px;
                left: 8.5px;
            }

            #LINE149 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }

            #LINE150 {
                width: 619px;
                top: 428.504px;
                left: 8.5px;
            }

            #LINE150 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }
            #LINE151 {
                width: 619px;
                top: 488.504px;
                left: 8.5px;
            }

            #LINE151 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }
            #LINE151 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48) !important;
                border-bottom: 2px solid rgb(232, 104, 48) !important;
                border-left: 2px solid rgb(232, 104, 48) !important;
            }

            #LINE152 {
                height: 490px;
                top: 8px;
                left: 435px;
            }
            #LINE152 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE152 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48);
                border-bottom: 2px solid rgb(232, 104, 48);
                border-left: 2px solid rgb(232, 104, 48);
            }
            #LINE153 {
                height: 490px;
                top: 8px;
                left: 0px;
            }
            #LINE153 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE153 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48);
                border-bottom: 2px solid rgb(232, 104, 48);
                border-left: 2px solid rgb(232, 104, 48);
            }
            #HEADLINE171 {
                width: 113px;
                top: 331.504px;
                left: 478px;
            }
            #HEADLINE171 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE172 {
                width: 113px;
                top: 391.504px;
                left: 478px;
            }
            #HEADLINE172 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE173 {
                width: 113px;
                top: 451.504px;
                left: 478px;
            }
            #HEADLINE173 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
        }

        .tcb-icon {
            display: inline-block;
            width: 4em;
            height: 1.7em;
            line-height: 1em;
            vertical-align: middle;
            stroke-width: 0;
            stroke: currentColor;
            fill: currentColor;
            -webkit-box-sizing: content-box;
            box-sizing: content-box;
        }

        .tcb-button-link span {
            color: rgb(255, 255, 255);
            --tcb-applied-color: #fff;
        }
        span.tcb-button-texts>span {
            display: block;
            padding: 0;
        }

        @media (max-width: 767px) {
            #FORM_ITEM281 {
                width: 335.416px;
                height: 132.207px;
                top: 10.375px;
                left: 0px;
            }

            #FORM_ITEM1997 {
                width: 335.724px;
                height: 132.207px;
                top: 182.379px;
                right: 0px;
            }

            #FORM_ITEM2002 {
                width: 335.724px;
                height: 132.207px;
                top: 351.379px;
                left: 0px;
            }

            .thrv_wrapper {
                width: 56px;
                height: 56px;
                border-radius: 50%;
                overflow: hidden;
                z-index: 3;
                position: relative;
                margin: 0px auto !important;
                top: 10.255px;
                left: -125.6px
            }
            .tve_image_caption {
                padding: 0;
                max-width: 100% !important;
                -webkit-box-sizing: border-box !important;
                box-sizing: border-box !important;
            }
            .tve_image_caption .tve_image_frame {
                display: block;
                max-width: 100%;
                position: relative;
                overflow: hidden;
            }
            .scrollRegister {
                top: 350.65px !important;
                left: 130px !important;
            }
            #LINE161 {
                height: 400px !important;
                top: 7.5px;
                left: 0px;
            }
            #LINE141 {
                height: 400px;
                top: 8px;
                left: 263.148px;
            }
            #LINE147 {
                height: 400px;
                top: 8px;
                left: 408.148px;
            }
            #LINE146 {
                width: 410px;
                top: 400px;
                left: 7.648px;
            }
            #HEADLINE118 {
                width: 403px;
                top: -9px;
                left: 29px;
            }
            #HEADLINE125 {
                top: 392px;
                left: 0px;
            }
            #HEADLINE125 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE126 {
                top: 445px;
                left: 0px;
            }
            #HEADLINE126 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE127 {
                top: 500px;
                left: 0px;
            }
            #HEADLINE127 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                text-align: justify;
                line-height: 1.6;
            }
            #LINE148 {
                height: 560px;
                top: 8px;
                left: 408.148px;
            }
            #LINE148 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE148 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48) !important;
                border-bottom: 2px solid rgb(232, 104, 48) !important;
                /* border-left: 2px solid rgb(232, 104, 48); */
            }
            #LINE149 {
                top: 453.504px;
                width: 410px;
                left: 7.648px
            }

            #LINE149 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }

            #LINE150 {
                top: 505.504px;
                width: 410px;
                left: 7.648px
            }

            #LINE150 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }
            #LINE151 {
                top: 560.504px;
                width: 410px;
                left: 7.648px
            }

            #LINE151 > .ladi-line {
                width: 100%;
                padding: 8px 0px;
            }
            #LINE151 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48) !important;
                border-bottom: 2px solid rgb(232, 104, 48) !important;
                border-left: 2px solid rgb(232, 104, 48) !important;
            }

            #LINE152 {
                height: 560px;
                top: 8px;
                left: 263.148px;
            }
            #LINE152 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE152 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48) !important;
                border-bottom: 2px solid rgb(232, 104, 48) !important;
                /* border-left: 2px solid rgb(232, 104, 48); */
            }
            #LINE153 {
                height: 560px;
                top: 8px;
                left: 0px;
            }
            #LINE153 > .ladi-line {
                height: 100%;
                padding: 0px 8px;
            }
            #LINE153 > .ladi-line > .ladi-line-container {
                border-top: 0px !important;
                border-right: 2px solid rgb(232, 104, 48);
                border-bottom: 2px solid rgb(232, 104, 48);
                border-left: 2px solid rgb(232, 104, 48);
            }
            #HEADLINE171 {
                width: 113px;
                top: 421.504px;
                left: 297.148px;
            }
            #HEADLINE171 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE172 {
                width: 113px;
                top: 475.504px;
                left: 297.148px;
            }
            #HEADLINE172 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
            #HEADLINE173 {
                width: 113px;
                top: 527.504px;
                left: 297.148px;
            }
            #HEADLINE173 > .ladi-headline {
                color: rgb(0, 0, 0);
                font-size: 16px;
                font-weight: bold;
                text-align: justify;
                line-height: 1.6;
            }
        }
    </style>
    <!-- End Facebook Pixel Code -->
</head>

<body>
    <div class="ladi-wraper">
        <div id="SECTION1" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="IMAGE2" class="ladi-element">
                    <div class='ladi-image'>
                        <div class="ladi-image-background"></div>
                    </div>
                </div>
                <div id="GROUP274" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="BUTTON4" class="ladi-element">
                            <div class='ladi-button ladi-transition'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT4" class="ladi-element">
                                    <p class='ladi-headline ladi-transition'>Trang Chủ</p>
                                </div>
                            </div>
                        </div>
                        <div data-action="true" id="BUTTON5" class="ladi-element">
                            <div class='ladi-button ladi-transition'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT5" class="ladi-element">
                                    <p class='ladi-headline ladi-transition'>Dịch vụ</p>
                                </div>
                            </div>
                        </div>
                        <div data-action="true" id="BUTTON7" class="ladi-element">
                            <div class='ladi-button ladi-transition'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT7" class="ladi-element">
                                    <p class='ladi-headline ladi-transition'>Bảng Giá</p>
                                </div>
                            </div>
                        </div>
                        <div data-action="true" id="BUTTON9" class="ladi-element">
                            <div class='ladi-button ladi-transition'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT9" class="ladi-element">
                                    <p class='ladi-headline ladi-transition'>Giới Thiệu</p>
                                </div>
                            </div>
                        </div>
                        <div data-action="true" id="BUTTON272" class="ladi-element">
                            <div class='ladi-button ladi-transition'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT272" class="ladi-element">
                                    <p class='ladi-headline ladi-transition'>Phản hồi</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div id="SECTION3" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP292" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="GROUP237" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="SHAPE92" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 24 24" fill="rgba(232, 105, 48, 1.0)"> <path d="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12.5,11A1.5,1.5 0 0,0 11,12.5A1.5,1.5 0 0,0 12.5,14A1.5,1.5 0 0,0 14,12.5A1.5,1.5 0 0,0 12.5,11M12,5A2,2 0 0,0 10,7A2,2 0 0,0 12,9A2,2 0 0,0 14,7A2,2 0 0,0 12,5M5.93,8.5C5.38,9.45 5.71,10.67 6.66,11.22C7.62,11.78 8.84,11.45 9.4,10.5C9.95,9.53 9.62,8.31 8.66,7.76C7.71,7.21 6.5,7.53 5.93,8.5Z"></path> </svg></div>
                                </div>
                                <div id="SHAPE93" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 24 24" fill="rgba(232, 106, 48, 0.8)"> <path d="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12.5,11A1.5,1.5 0 0,0 11,12.5A1.5,1.5 0 0,0 12.5,14A1.5,1.5 0 0,0 14,12.5A1.5,1.5 0 0,0 12.5,11M12,5A2,2 0 0,0 10,7A2,2 0 0,0 12,9A2,2 0 0,0 14,7A2,2 0 0,0 12,5M5.93,8.5C5.38,9.45 5.71,10.67 6.66,11.22C7.62,11.78 8.84,11.45 9.4,10.5C9.95,9.53 9.62,8.31 8.66,7.76C7.71,7.21 6.5,7.53 5.93,8.5Z"></path> </svg></div>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP238" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE17" class="ladi-element">
                                    <h3 class='ladi-headline'>GIỚI THIỆU VỀ DỊCH VỤ SHOPEE</h3>
                                </div>
                                <div id="HEADLINE21" class="ladi-element">
                                    <h3 class='ladi-headline'>Cung cấp dịch vụ hỗ trợ bán hàng hiệu quả trên Shopee&nbsp;</h3>
                                </div>
                            </div>
                        </div>
                        <div id="HEADLINE78" class="ladi-element">
                            <h3 class='ladi-headline'>Tăng lượng bán số lượng lớn kèm đánh giá chất lượng sản phẩm</h3>
                        </div>
                        <div id="HEADLINE79" class="ladi-element">
                            <h3 class='ladi-headline'>Đẩy top sản phẩm tìm kiếm&nbsp;</h3>
                        </div>
                        <div id="HEADLINE80" class="ladi-element">
                            <h3 class='ladi-headline'>Tăng follow, lượt thích và lượt xem sản phẩm&nbsp;</h3>
                        </div>
                        <div id="BOX86" class="ladi-element">
                            <div class='ladi-box'></div>
                            <div class="ladi-overlay"></div>
                        </div>
                        <div id="HEADLINE24" class="ladi-element">
                            <h3 class='ladi-headline'>TĂNG TƯƠNG TÁC</h3>
                        </div>
                        <div id="HEADLINE81" class="ladi-element">
                            <h3 class='ladi-headline'>Bán hàng ra đơn nhanh chóng</h3>
                        </div>
                        <div id="BOX85" class="ladi-element">
                            <div class='ladi-box'></div>
                            <div class="ladi-overlay"></div>
                        </div>
                        <div id="HEADLINE23" class="ladi-element">
                            <h3 class='ladi-headline'>BUFF ĐƠN</h3>
                        </div>
                        <div id="BOX83" class="ladi-element">
                            <div class='ladi-box'></div>
                            <div class="ladi-overlay"></div>
                        </div>
                        <div id="HEADLINE22" class="ladi-element">
                            <h3 class='ladi-headline'>&nbsp;BUFF ĐƠN&nbsp;</h3>
                        </div>
                        <div id="BOX87" class="ladi-element">
                            <div class='ladi-box'></div>
                            <div class="ladi-overlay"></div>
                        </div>
                        <div id="HEADLINE25" class="ladi-element">
                            <h3 class='ladi-headline'>TƯ VẤN</h3>
                        </div>
                    </div>
                </div>
                <a href="tel:0978093540" id="IMAGE271" class="ladi-element">
                    <div class='ladi-image'>
                        <div class="ladi-image-background"></div>
                    </div>
                </a>
                <div data-action="true" id="GROUP297" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="SHAPE295" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1280 1896.0833" fill="rgba(232, 105, 48, 1.0)"> <path d="M1107 877l-742 742q-19 19-45 19t-45-19l-166-166q-19-19-19-45t19-45l531-531-531-531q-19-19-19-45t19-45L275 45q19-19 45-19t45 19l742 742q19 19 19 45t-19 45z"></path> </svg></div>
                        </div>
                        <div id="SHAPE296" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1280 1896.0833" fill="rgba(232, 106, 48, 0.8)"> <path d="M1107 877l-742 742q-19 19-45 19t-45-19l-166-166q-19-19-19-45t19-45l531-531-531-531q-19-19-19-45t19-45L275 45q19-19 45-19t45 19l742 742q19 19 19 45t-19 45z"></path> </svg></div>
                        </div>
                    </div>
                </div>
                <div id="GROUP244" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE13" class="ladi-element">
                            <h1 class='ladi-headline'>DỊCH VỤ SHOPEE</h1>
                        </div>
                        <div id="GROUP240" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="BOX76" class="ladi-element">
                                    <div class='ladi-box'></div>
                                    <div class="ladi-overlay"></div>
                                </div>
                                <div id="HEADLINE14" class="ladi-element">
                                    <h3 class='ladi-headline'>AN TOÀN - UY TÍN - CHUYÊN NGHIỆP&nbsp;</h3>
                                </div>
                            </div>
                        </div>
                        <div data-action="true" id="BUTTON15" class="ladi-element">
                            <div class='ladi-button'>
                                <div class="ladi-button-background"></div>
                                <div id="BUTTON_TEXT15" class="ladi-element">
                                    <p class='ladi-headline'>Đăng Kí Ngay</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION19" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP246" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="GROUP232" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="SHAPE192" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 186 178.97" fill="rgba(232, 109, 48, 1.0)"><path d="M185,79.16a21.34,21.34,0,0,0-13.33-13.55,21.32,21.32,0,0,0-24.05-32.5c-8.62,2.78-18.91,11.8-28.67,22.4,7-12.54,12.44-25.09,12.43-34.17A21.31,21.31,0,0,0,93,8.48,21.32,21.32,0,0,0,54.66,21.32c0,9.07,5.39,21.65,12.47,34.22-7.24-7.86-18.61-19.1-28.69-22.43a21.32,21.32,0,0,0-24.07,32.5A21.32,21.32,0,0,0,14.74,106c10,3.28,26,.8,36.36-1.28-9.31,4.27-24.28,12-30.17,20.36A21.32,21.32,0,0,0,44.4,158,21.33,21.33,0,0,0,83,170.19c5.35-7.32,8.37-20.69,10-35,1.23,10.55,3.84,26.46,10,35A21.32,21.32,0,0,0,141.59,158a21.31,21.31,0,0,0,23.47-32.91c-5.32-7.35-17.09-14.35-30.21-20.37,10.47,2.11,26.24,4.54,36.4,1.29A21.32,21.32,0,0,0,185,79.16ZM58.31,21.33a17.67,17.67,0,0,1,33.1-8.63,1.83,1.83,0,0,0,3.19,0,17.67,17.67,0,0,1,33.09,8.63c0,15-17,41.74-27.16,56.36-.2.26-.39.5-.58.76a14.34,14.34,0,0,0-5.12-1.66H91.18A14.38,14.38,0,0,0,86,78.45l-.56-.75C75.31,63.05,58.28,36.28,58.31,21.33ZM15.87,102.57a17.68,17.68,0,0,1,2-34.15,1.82,1.82,0,0,0,1-3,17.67,17.67,0,0,1,18.46-28.8c14.24,4.61,34.42,29.07,45.2,43.26l.54.77A15.11,15.11,0,0,0,79.9,85l-1.11,3.47a14,14,0,0,0,0,5.39c-.7.25-.74.25-.8.27C60.86,99.29,30.1,107.24,15.87,102.57ZM80,168a17.68,17.68,0,0,1-31.86-12.49,1.83,1.83,0,0,0-2.58-1.86A17.68,17.68,0,0,1,23.9,127.24c8.8-12.13,38.22-23.74,55.18-29.65l.83-.24a14.91,14.91,0,0,0,3.17,4.36l3,2.14a14.55,14.55,0,0,0,5.13,1.67c0,1.36,0,1,0,1.17C90.79,124.45,88.81,156,80,168Zm2.66-73.58a10.83,10.83,0,1,1,20.61.08,11,11,0,0,1-3.93,5.38A10.82,10.82,0,0,1,82.69,94.46Zm79.42,32.8a17.67,17.67,0,0,1-21.71,26.43,1.82,1.82,0,0,0-2.57,1.86A17.67,17.67,0,0,1,106,168c-8.78-12.08-10.75-43.63-11.14-61.32,0,0,0-1.15,0-1.18a14.48,14.48,0,0,0,5.12-1.67l3-2.14a14.84,14.84,0,0,0,3.17-4.37l.9.27C123.82,103.49,153.33,115.14,162.11,127.26Zm18.31-33.48a17.58,17.58,0,0,1-10.31,8.8c-14.26,4.64-45-3.29-62-8.44l-.87-.3a13.88,13.88,0,0,0,0-5.38L106.1,85a14.68,14.68,0,0,0-3.17-4.37l.55-.79c10.79-14.2,31-38.66,45.22-43.25a17.67,17.67,0,0,1,18.42,28.81,1.83,1.83,0,0,0,1,3A17.67,17.67,0,0,1,180.42,93.78Z"></path></svg></div>
                                </div>
                                <div id="HEADLINE27" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ NỔI BẬT&nbsp;</h3>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP231" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE100" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ TĂNG TƯƠNG TÁC</h3>
                                </div>
                                <div id="SHAPE101" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 156.3 150" fill="rgba(232, 106, 48, 1)"><path d="M125.5,0H30.9A30.5,30.5,0,0,0,.1,33.1l6.2,83.7c1.3,17.8,16.6,32.4,34.3,33.1h73.8c18.3,0,34.3-14.9,35.6-33.1l6.2-83.7A30.5,30.5,0,0,0,125.5,0Z"></path></svg></div>
                                </div>
                                <div id="HEADLINE102" class="ladi-element">
                                    <h3 class='ladi-headline'>Tăng lượt follow cho shop theo yêu cầu.</h3>
                                </div>
                                <div id="HEADLINE103" class="ladi-element">
                                    <h3 class='ladi-headline'>Tăng lượt thích cho toàn bộ sản phẩm.</h3>
                                </div>
                                <div id="HEADLINE104" class="ladi-element">
                                    <h3 class='ladi-headline'>Tăng lượt xem sản phẩm, đẩy tương tác</h3>
                                </div>
                                <div id="SHAPE108" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 24 24" fill="rgba(255, 255, 255, 1)"> <path d="M11,9H13V6H16V4H13V1H11V4H8V6H11M7,18A2,2 0 0,0 5,20A2,2 0 0,0 7,22A2,2 0 0,0 9,20A2,2 0 0,0 7,18M17,18A2,2 0 0,0 15,20A2,2 0 0,0 17,22A2,2 0 0,0 19,20A2,2 0 0,0 17,18M7.17,14.75L7.2,14.63L8.1,13H15.55C16.3,13 16.96,12.59 17.3,11.97L21.16,4.96L19.42,4H19.41L18.31,6L15.55,11H8.53L8.4,10.73L6.16,6L5.21,4L4.27,2H1V4H3L6.6,11.59L5.25,14.04C5.09,14.32 5,14.65 5,15A2,2 0 0,0 7,17H19V15H7.42C7.29,15 7.17,14.89 7.17,14.75Z"></path> </svg></div>
                                </div>
                                <div id="SHAPE109" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1792 1896.0833" fill="rgba(255, 255, 255, 1)"> <path d="M896 1664q-26 0-44-18l-624-602q-10-8-27.5-26T145 952.5 77 855 23.5 734 0 596q0-220 127-344t351-124q62 0 126.5 21.5t120 58T820 276t76 68q36-36 76-68t95.5-68.5 120-58T1314 128q224 0 351 124t127 344q0 221-229 450l-623 600q-18 18-44 18z"></path> </svg></div>
                                </div>
                                <div id="SHAPE110" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1920 1896.0833" fill="rgba(255, 255, 255, 1)"> <path d="M593 896q-162 5-265 128H194q-82 0-138-40.5T0 865q0-353 124-353 6 0 43.5 21t97.5 42.5T384 597q67 0 133-23-5 37-5 66 0 139 81 256zm1071 637q0 120-73 189.5t-194 69.5H523q-121 0-194-69.5T256 1533q0-53 3.5-103.5t14-109T300 1212t43-97.5 62-81 85.5-53.5T602 960q10 0 43 21.5t73 48 107 48 135 21.5 135-21.5 107-48 73-48 43-21.5q61 0 111.5 20t85.5 53.5 62 81 43 97.5 26.5 108.5 14 109 3.5 103.5zM640 256q0 106-75 181t-181 75-181-75-75-181 75-181T384 0t181 75 75 181zm704 384q0 159-112.5 271.5T960 1024 688.5 911.5 576 640t112.5-271.5T960 256t271.5 112.5T1344 640zm576 225q0 78-56 118.5t-138 40.5h-134q-103-123-265-128 81-117 81-256 0-29-5-66 66 23 133 23 59 0 119-21.5t97.5-42.5 43.5-21q124 0 124 353zm-128-609q0 106-75 181t-181 75-181-75-75-181 75-181 181-75 181 75 75 181z"></path> </svg></div>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP230" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="SHAPE95" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 156.3 150" fill="rgba(232, 106, 48, 1)"><path d="M125.5,0H30.9A30.5,30.5,0,0,0,.1,33.1l6.2,83.7c1.3,17.8,16.6,32.4,34.3,33.1h73.8c18.3,0,34.3-14.9,35.6-33.1l6.2-83.7A30.5,30.5,0,0,0,125.5,0Z"></path></svg></div>
                                </div>
                                <div id="HEADLINE96" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ BUFF ĐƠN</h3>
                                </div>
                                <div id="HEADLINE99" class="ladi-element">
                                    <h3 class='ladi-headline'>Giúp tăng tỉ lệ hiển thị sản phẩm với nhiều khách hàng hơn&nbsp;</h3>
                                </div>
                                <div id="SHAPE107" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 2048 1896.0833" fill="rgba(255, 255, 255, 1)"> <path d="M704 896q-159 0-271.5-112.5T320 512t112.5-271.5T704 128t271.5 112.5T1088 512 975.5 783.5 704 896zm960 128h352q13 0 22.5 9.5t9.5 22.5v192q0 13-9.5 22.5t-22.5 9.5h-352v352q0 13-9.5 22.5t-22.5 9.5h-192q-13 0-22.5-9.5t-9.5-22.5v-352h-352q-13 0-22.5-9.5t-9.5-22.5v-192q0-13 9.5-22.5t22.5-9.5h352V672q0-13 9.5-22.5t22.5-9.5h192q13 0 22.5 9.5t9.5 22.5v352zm-736 224q0 52 38 90t90 38h256v238q-68 50-171 50H267q-121 0-194-69T0 1405q0-53 3.5-103.5t14-109T44 1084t43-97.5 62-81 85.5-53.5T346 832q19 0 39 17 79 61 154.5 91.5T704 971t164.5-30.5T1023 849q20-17 39-17 132 0 217 96h-223q-52 0-90 38t-38 90v192z"></path> </svg></div>
                                </div>
                                <div id="HEADLINE97" class="ladi-element">
                                    <h3 class='ladi-headline'>Giúp sản phẩm có nhiều lượt bán + đánh giá chất lượng 5* trong thời gian ngắn.</h3>
                                </div>
                                <div id="SHAPE105" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1664 1896.0833" fill="rgba(255, 255, 255, 1)"> <path d="M1664 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5T1321 1619q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5T301 1569q0-6 2-20l86-500L25 695Q0 668 0 647q0-37 56-46l502-73L783 73q19-41 49-41t49 41l225 455 502 73q56 9 56 46z"></path> </svg></div>
                                </div>
                                <div id="HEADLINE98" class="ladi-element">
                                    <h3 class='ladi-headline'>Khách hàng sẽ cảm thấy yên tâm và tin tưởng khi đưa ra quyết định mua hàng của shop anh/chị.</h3>
                                </div>
                                <div id="SHAPE106" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 24 24" fill="rgba(255, 255, 255, 1)"> <path d="M12,18A6,6 0 0,1 6,12A6,6 0 0,1 12,6A6,6 0 0,1 18,12A6,6 0 0,1 12,18M20,15.31L23.31,12L20,8.69V4H15.31L12,0.69L8.69,4H4V8.69L0.69,12L4,15.31V20H8.69L12,23.31L15.31,20H20V15.31Z"></path> </svg></div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION30" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP227" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="GROUP224" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE116" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ BUFF ĐƠN ẢO TĂNG DOANH SỐ BÁN HÀNG</h3>
                                </div>
                                <div id="GROUP219" class="ladi-element">
                                    <div class='ladi-group'>
                                        <div id="LINE141" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE142" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE143" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE144" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE145" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE146" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE147" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE148" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE149" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE150" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE151" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE152" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE153" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="GROUP132" class="ladi-element">
                                            <div class='ladi-group'>
                                                <div id="HEADLINE120" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 1: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 20 comment đánh giá chất lượng 5* ~200 lượt bán</span></h3>
                                                </div>
                                                <div id="HEADLINE121" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 2: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 30 comment đánh giá chất lượng 5* ~400 lượt bán</span></h3>
                                                </div>
                                                <div id="HEADLINE122" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 3: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 40 comment đánh giá chất lượng 5* ~600 lượt bán</span></h3>
                                                </div>
                                                <div id="HEADLINE123" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 4: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 60 comment đánh giá chất lượng 5* ~800 lượt bán</span></h3>
                                                </div>
                                                <div id="HEADLINE124" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 5:</span> Buff 10-20 sản có 80 comment đánh giá chất lượng 5* ~1200 lượt bán</h3>
                                                </div>
                                                <div id="HEADLINE125" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói đua top 1:</span> Buff 1-5 sản phẩm ~ 1000 lượt bán</h3>
                                                </div>
                                                <div id="HEADLINE126" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói đua top 2:</span> Buff 1-5 sản phẩm 2000 lượt bán</h3>
                                                </div>
                                                <div id="HEADLINE127" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói đua top 3:</span> Buff 1-5 sản phẩm 5000 lượt bán</h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="LINE158" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE161" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="HEADLINE166" class="ladi-element">
                                            <h3 class='ladi-headline'>300.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE167" class="ladi-element">
                                            <h3 class='ladi-headline'>450.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE168" class="ladi-element">
                                            <h3 class='ladi-headline'>600.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE169" class="ladi-element">
                                            <h3 class='ladi-headline'>900.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE170" class="ladi-element">
                                            <h3 class='ladi-headline'>1.200.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE171" class="ladi-element">
                                            <h3 class='ladi-headline'>700.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE172" class="ladi-element">
                                            <h3 class='ladi-headline'>1.400.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE173" class="ladi-element">
                                            <h3 class='ladi-headline'>3.000.000 VNĐ<br></h3>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP223" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE118" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ TĂNG FOLLOW, LIKE, VIEW (có thể thanh toán sau với shop uy tín)</h3>
                                </div>
                                <div id="GROUP220" class="ladi-element">
                                    <div class='ladi-group'>
                                        <div id="LINE141" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE142" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE143" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE144" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE145" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE146" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE147" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="GROUP132" class="ladi-element">
                                            <div class='ladi-group'>
                                                <div id="HEADLINE120" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 1: <span style="font-weight: normal;">Tăng 2000 follow và khuyến mãi 10k lượt like + view toàn bộ sản phẩm</span></h3>
                                                </div>
                                                <div id="HEADLINE121" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 2: <span style="font-weight: normal;">Tăng 3000 follow và khuyến mãi 20 ~ 35k lượt like + view toàn bộ sản phẩm</span></h3>
                                                </div>
                                                <div id="HEADLINE122" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 3: <span style="font-weight: normal;">Tăng 4000 follow và khuyến mãi 30k ~ 45k lượt like + view toàn bộ sản phẩm</span></h3>
                                                </div>
                                                <div id="HEADLINE123" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 4: <span style="font-weight: normal;">Tăng 5500 follow và khuyến mãi 40k ~ 55k lượt like + view toàn bộ sản phẩm</span></h3>
                                                </div>
                                                <div id="HEADLINE124" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 5:</span> Tăng 10000 follow và khuyến mãi 100k ~ 150k lượt like + view toàn bộ sản phẩm</h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="LINE158" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE161" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="HEADLINE166" class="ladi-element">
                                            <h3 class='ladi-headline'>200.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE167" class="ladi-element">
                                            <h3 class='ladi-headline'>300.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE168" class="ladi-element">
                                            <h3 class='ladi-headline'>350.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE169" class="ladi-element">
                                            <h3 class='ladi-headline'>450.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE170" class="ladi-element">
                                            <h3 class='ladi-headline'>800.000 VNĐ<br></h3>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP228" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE116" class="ladi-element">
                                    <h3 class='ladi-headline'>DỊCH VỤ BUFF ĐƠN ẢO LAZADA TĂNG DOANH SỐ BÁN HÀNG</h3>
                                </div>
                                <div id="GROUP219" class="ladi-element">
                                    <div class='ladi-group'>
                                        <div id="LINE141" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE142" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE143" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE144" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE145" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE146" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE147" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE148" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE149" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE150" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE151" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE152" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE153" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="GROUP132" class="ladi-element">
                                            <div class='ladi-group'>
                                                <div id="HEADLINE120" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 1: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 30 comment đánh giá chất lượng 5*</span></h3>
                                                </div>
                                                <div id="HEADLINE121" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 2: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 40 comment đánh giá chất lượng 5*</span></h3>
                                                </div>
                                                <div id="HEADLINE122" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 3: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 50 comment đánh giá chất lượng 5*</span></h3>
                                                </div>
                                                <div id="HEADLINE123" class="ladi-element">
                                                    <h3 class='ladi-headline'>Gói ưu đãi 4: <span style="font-weight: normal;">Buff 10-20 sản phẩm có 70 comment đánh giá chất lượng 5*</span></h3>
                                                </div>
                                                <div id="HEADLINE124" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 5:</span> Buff 10-20 sản phẩm có 90 comment đánh giá chất lượng 5*</h3>
                                                </div>
                                                <div id="HEADLINE125" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 6:</span> Buff 5 sản phẩm có 50 đánh giá 5*</h3>
                                                </div>
                                                <div id="HEADLINE126" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 7:</span> Buff 10 sản phẩm có 100 đánh giá 5*</h3>
                                                </div>
                                                <div id="HEADLINE127" class="ladi-element">
                                                    <h3 class='ladi-headline'><span style="font-weight: bold;">Gói ưu đãi 8:</span> Buff 10 sản phẩm có 200 đánh giá 5*</h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="LINE158" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="LINE161" class="ladi-element">
                                            <div class='ladi-line'>
                                                <div class="ladi-line-container"></div>
                                            </div>
                                        </div>
                                        <div id="HEADLINE166" class="ladi-element">
                                            <h3 class='ladi-headline'>300.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE167" class="ladi-element">
                                            <h3 class='ladi-headline'>450.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE168" class="ladi-element">
                                            <h3 class='ladi-headline'>600.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE169" class="ladi-element">
                                            <h3 class='ladi-headline'>900.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE170" class="ladi-element">
                                            <h3 class='ladi-headline'>1.200.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE171" class="ladi-element">
                                            <h3 class='ladi-headline'>700.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE172" class="ladi-element">
                                            <h3 class='ladi-headline'>1.400.000 VNĐ<br></h3>
                                        </div>
                                        <div id="HEADLINE173" class="ladi-element">
                                            <h3 class='ladi-headline'>3.000.000 VNĐ<br></h3>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="GROUP225" class="ladi-element">
                            <div class='ladi-group'>
                                <div data-action="true" id="BUTTON15" onclick="scrollRegister()" class="ladi-element scrollRegister">
                                    <div class='ladi-button'>
                                        <div class="ladi-button-background"></div>
                                        <div id="BUTTON_TEXT15" class="ladi-element">
                                            <p class='ladi-headline'>Đăng Kí Ngay</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="GROUP299" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="GROUP226" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="HEADLINE32" class="ladi-element">
                                    <h3 class='ladi-headline'>BẢNG GIÁ VÀ GÓI DỊCH VỤ&nbsp;</h3>
                                </div>
                                <div id="SHAPE175" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 156.3 150" fill="rgba(232, 107, 48, 1.0)"><path d="M125.5,0H30.9A30.5,30.5,0,0,0,.1,33.1l6.2,83.7c1.3,17.8,16.6,32.4,34.3,33.1h73.8c18.3,0,34.3-14.9,35.6-33.1l6.2-83.7A30.5,30.5,0,0,0,125.5,0Z"></path></svg></div>
                                </div>
                                <div id="SHAPE176" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 156.3 150" fill="rgba(240, 238, 175) 1.0)"><path d="M125.5,2a28.5,28.5,0,0,1,28.8,30.9l-6.2,83.7c-1.2,17.2-16.3,31.3-33.6,31.3H40.7a33.8,33.8,0,0,1-22-9.6A33.6,33.6,0,0,1,8.3,116.7L2.1,33A28.5,28.5,0,0,1,30.9,2h94.6m0-2H30.9A30.5,30.5,0,0,0,.1,33.1l6.2,83.7c1.3,17.8,16.6,32.4,34.3,33.1h73.8c18.3,0,34.3-14.9,35.6-33.1l6.2-83.7A30.5,30.5,0,0,0,125.5,0Z"></path></svg></div>
                                </div>
                            </div>
                        </div>
                        <div id="LINE263" class="ladi-element">
                            <div class='ladi-line'>
                                <div class="ladi-line-container"></div>
                            </div>
                        </div>
                        <div id="LINE264" class="ladi-element">
                            <div class='ladi-line'>
                                <div class="ladi-line-container"></div>
                            </div>
                        </div>
                        <div id="LINE265" class="ladi-element">
                            <div class='ladi-line'>
                                <div class="ladi-line-container"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION31" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="SHAPE266" class="ladi-element">
                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 416.07 97.96" fill="rgba(240, 238, 175) 1.0)"><path d="M415.93,69.33c-8.52,0-18.6.57-28,.57,0-1,2.1-2.42,2.87-3,1.16-.85,3.13-.63,4.56-3.17.44-.77-1.24-1.84,0-3.23.74-.84,2-1,2.56-1.48.74-.61,7.84-4.58,2-4.58.39-2.15,7.7-1,7.44-3-.08-.65-19.25-1-21.17-1,0-2.35,5.1,0,5.72-1.14.83-1.52-7.66-1.12-8.57-1.91.82-2.44.88.25-.62-2.46,5.08-4.78,11.85-2.2,17.75-5.61-1.82-1.61-8.17.68-10.5.82-4.22.26-13.52,1.43-16.36-1.42,1.42-1.48,9.88-.82,10.19-1.63.73-1.91-10.36.43-10.77-2.1.89.27,1.44-1.74.81-2.28.77.66-4.6,0-4.25-1.15.93.41,1.5.22,1.74-.56,0-1.2-6.2-1.73-6.89-1.73,0-3,12.9-4.62,14.82-4.87,3.53-.44,8.16-1.21,11.51-1.42,1.6-.11,4-.51,5.15-.58,2.11-.12,4,.2,5.79-.45,1-.36-.2-1.26-.64-1.26,0-.68,4.94-3.21,5.72-3.44,2.43-.7,6.7.53,8.51-1.39-1-1.1-10.83-.83-12.38-.9-4.3-.19-8.14-.07-12.08-.57-7.41-.94-14.46-1.16-21.82-2.29-12.21-1.87-24.64-4.48-37.49-5.72C317,5,302,3.93,287.31,3.09,271,2.15,253.49.31,236.68.09,213-.23,190.44.39,167.56.66c-14.71.17-31.73-.19-46.35,1.71-28,3.65-57.48,3-85.56,8-4.31.77-27.23,6.23-18.34,12,1.34.87,4,.65,4.9,1.43.68.58,2.71,4.31,2.58,5.43-.21,1.85,1,.89-.67,2.87-.43.52-3.82-.58-4.2,0-1.48,2.29,4.52,1.15,5.72,1.15,0,2.69-5.88.59-6.78,2.49-.11.25,13-1.16,11.72,2.43-.73,2.1-6-1.26-7.13,1-.17.34,5.7,1.1,6.2,1-.07.43-10.8,1.15-12,1.15-.25.82,1.77,1.47,2.3,1.91-.3,3.19-8.21-1.11-5.61,3.81,1.79,3.38,6,1.51,8.46,3.16-1.94,2.67-1.87.74-1.49,4.28.18,1.64,3.44.37.55,2.87-1.38,1.19-7.46,1.19-9.65,1.14-3.48-.08-7.22-1-10,.84a12.6,12.6,0,0,0-2,2c-.34.67-.23,4,.57,4.58,1.1.85,3.37-.19,4.29.57,2.25,1.84-.16,3.44.88,5.15.55.89,3.59,3.73,4.27,4.58.52.64,1.88,1.87,2.6,2.91,1,1.43.62,2.44,1.89,3.56,2.24,2,5.66,2.22,8.12,4.14,1.86,1.45,2.29,2.41,4.5,4,2.76,1.95-.82,2,4.43,2.48,0,0,.82-1.06,1-1,2.33.2,13.82,2.29,13.57,2.56.65.87,2.27.06,2.27-.85,2.81,0,5.37,1.4,8.29,1.72,1.28.14,2.28-.36,4,.58.81-.92,5.69.54,7.11.56,2.77,0,20.54.82,20,.85,2.29-.15,10.53-1,11.12.31,1.52-1.44,11.37-1.44,13.78-1.73,3.7-.45,7.47-.13,10.3-.58,5-.77,9.58,0,13.73.58,2.6.33,143.42-.78,168.09-1.72,10.88-.42,25.81,0,36.28,0,3.54,0,34,1.06,34,0,0-.48-7.66-1.17-8.11-1.15a24.85,24.85,0,0,1-5.85.09,51.74,51.74,0,0,0-7.23-.37,35.42,35.42,0,0,1-4.83.29,26.57,26.57,0,0,1-2.8-1.06c-1-.21-1.16,1.34-2.1,1.06a.9.9,0,0,0-.57-1.15c.43-2.17,24.68-.6,26.33-.57,1.29,0,2.64-.5,4.22-.5a66.55,66.55,0,0,1,6.67.2,164.81,164.81,0,0,0,18.55-.27c1,0,19.52-4.74,16-7.44h-4.28C411.35,82.05,416.34,69.33,415.93,69.33Z"></path></svg></div>
                </div>
                <div id="SHAPE249" class="ladi-element">
                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 91.27 116.66" fill="rgba(232, 105, 48, 1.0)"><path d="M86.47,71.48C77.36,65.27,58.31,54.77,25.4,47-11.3,35.23-1.53,8.28,13.16,1.67c18.63-8.38,63.48,15,78,66.36A3.08,3.08,0,0,1,86.47,71.48Z"></path><path d="M69.44,83.29c-6,.38-17.76,2.24-34.58,9.92-19.76,7.25-24.6-7.65-20.48-15.43,5.23-9.87,32.41-14.85,55.91,2.45A1.69,1.69,0,0,1,69.44,83.29Z"></path><path d="M74.36,96.89C71.12,98.76,65.1,103,57.86,112c-9,9.5-15.81,2.54-15.67-2.93C42.38,102.09,56.13,91.82,74,95A1.05,1.05,0,0,1,74.36,96.89Z"></path></svg></div>
                </div>
                <div id="HEADLINE35" class="ladi-element">
                    <h3 class='ladi-headline'>FEEDBACK VỀ SẢN PHẨM</h3>
                </div>
                <div id="GROUP198" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="BOX179" class="ladi-element">
                            <div class='ladi-box'></div>
                            <div class="ladi-overlay"></div>
                        </div>
                        <div id="HEADLINE37" class="ladi-element">
                            <h3 class='ladi-headline'>Dịch vụ SHOPEE đã đươc rất nhiều khách hàng yên tâm, tin tưởng hợp tác.
                                <br>Các khách hàng đều thu được kết quả bán hàng rất tốt sau khi sử dụng dịch vụ của chúng tôi
                                <br>Sự tin tưởng của khách hàng chính là động lực để chúng tôi nỗ lực phát triển hơn nữa mang đến những lợi ích tốt nhất cho khách hàng.&nbsp;<br></h3>
                        </div>
                        <div id="SHAPE177" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 129.34 107.78" fill="rgba(232, 105, 48, 1.0)"><polygon points="0 107.78 0 50.18 24.87 0 55.85 0 35.35 48.44 36.66 50.18 56.73 50.18 56.73 107.78 0 107.78"></polygon><polygon points="72.61 107.78 72.61 50.18 97.48 0 128.47 0 107.96 48.44 109.27 50.18 129.34 50.18 129.34 107.78 72.61 107.78"></polygon></svg></div>
                        </div>
                        <div id="SHAPE178" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 129.34 107.78" fill="rgba(232, 105, 48, 1.0)"><polygon points="0 107.78 0 50.18 24.87 0 55.85 0 35.35 48.44 36.66 50.18 56.73 50.18 56.73 107.78 0 107.78"></polygon><polygon points="72.61 107.78 72.61 50.18 97.48 0 128.47 0 107.96 48.44 109.27 50.18 129.34 50.18 129.34 107.78 72.61 107.78"></polygon></svg></div>
                        </div>
                    </div>
                </div>
                <div id="HEADLINE284" class="ladi-element">
                    <h3 class='ladi-headline'>+10000 người đang sử dụng dịch vụ SHOPEE</h3>
                </div>
                <div id="GROUP317" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="SHAPE314" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(210, 210, 210, 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="SHAPE313" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(240, 238, 175) 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="IMAGE309" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                        <div id="HEADLINE315" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng Xuân Lưu – 23 tuổi – Kinh doanh thuốc giảm cân&nbsp;</h3>
                        </div>
                        <div id="HEADLINE316" class="ladi-element">
                            <h3 class='ladi-headline'>“Các anh làm dịch vụ rất hay mà giờ em mới biết đến, chạy thử follow và buff thử 1 gói lượt bán, đánh giá 5* xem thế nào thì ra đơn thật luôn, feedback sp rất có tâm. Đợt này shop có sp mới lại nhờ các anh buff đơn tiếp nhé”</h3>
                        </div>
                    </div>
                </div>
                <div id="GROUP318" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="SHAPE319" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(210, 210, 210, 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="SHAPE320" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(240, 238, 175) 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="IMAGE321" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                        <div id="HEADLINE322" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng Lê Linh - 27 tuổi – Kinh doanh điện thoại</h3>
                        </div>
                        <div id="HEADLINE323" class="ladi-element">
                            <h3 class='ladi-headline'>“Mình bán điện thoại giá sản phẩm khá đắt nếu không có đánh giá và lượt bán sẽ khó có người mua. Sau khi sử dụng gói buff lượt bán và đánh giá bên bạn thì lên hẳn top 3 luôn ra đơn ầm ầm. Cảm ơn các bạn nhé”</h3>
                        </div>
                    </div>
                </div>
                <div id="GROUP324" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="SHAPE325" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(210, 210, 210, 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="SHAPE326" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(240, 238, 175) 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="IMAGE327" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                        <div id="HEADLINE328" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng Văn Sản – 23 tuổi – Kinh doanh thuốc thú y</h3>
                        </div>
                        <div id="HEADLINE329" class="ladi-element">
                            <h3 class='ladi-headline'>&nbsp;Dịch vụ uy tín, lúc đầu mới làm cũng hơi lo vì phải chuyển tiền hàng nhưng sau tiền về ví shopee như đúng cam kết, thở phào vì ko đặt niềm tin nhầm chỗ .Vote cho team nhé</h3>
                        </div>
                    </div>
                </div>
                <div id="GROUP330" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="SHAPE331" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(210, 210, 210, 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="SHAPE332" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 150.2 150" fill="rgba(240, 238, 175) 1.0)"><path d="M0,43.7V106a44,44,0,0,0,44,44h62.3a44,44,0,0,0,44-44V0H39.4A44,44,0,0,0,0,43.7Z"></path></svg></div>
                        </div>
                        <div id="IMAGE333" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                        <div id="HEADLINE334" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng Minh Thúy – 22 tuổi – Kinh doanh mĩ phẩm&nbsp;</h3>
                        </div>
                        <div id="HEADLINE335" class="ladi-element">
                            <h3 class='ladi-headline'>“Dịch vụ chuyên nghiệp, uy tín, buff 2 gói xong thấy hiệu quả luôn, 1 ngày ít nhất 8 đơn, không biết đến dịch vụ của các bạn sớm hơn”</h3>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION49" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP301" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE50" class="ladi-element">
                            <h3 class='ladi-headline'>10K</h3>
                        </div>
                        <div id="HEADLINE58" class="ladi-element">
                            <h3 class='ladi-headline'>Người sử dụng</h3>
                        </div>
                        <div id="SHAPE59" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 24 24" fill="rgba(232, 105, 48, 1.0)"> <path d="M17,13L12,18L7,13H10V9H14V13M19.35,10.03C18.67,6.59 15.64,4 12,4C9.11,4 6.6,5.64 5.35,8.03C2.34,8.36 0,10.9 0,14A6,6 0 0,0 6,20H19A5,5 0 0,0 24,15C24,12.36 21.95,10.22 19.35,10.03Z"></path> </svg></div>
                        </div>
                    </div>
                </div>
                <div id="GROUP302" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE51" class="ladi-element">
                            <h3 class='ladi-headline'>8K</h3>
                        </div>
                        <div id="HEADLINE55" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng hạnh phúc</h3>
                        </div>
                        <div id="SHAPE60" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1792 1896.0833" fill="rgba(232, 105, 48, 1.0)"> <path d="M896 1664q-26 0-44-18l-624-602q-10-8-27.5-26T145 952.5 77 855 23.5 734 0 596q0-220 127-344t351-124q62 0 126.5 21.5t120 58T820 276t76 68q36-36 76-68t95.5-68.5 120-58T1314 128q224 0 351 124t127 344q0 221-229 450l-623 600q-18 18-44 18z"></path> </svg></div>
                        </div>
                    </div>
                </div>
                <div id="GROUP303" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE52" class="ladi-element">
                            <h3 class='ladi-headline'>5.4K</h3>
                        </div>
                        <div id="HEADLINE56" class="ladi-element">
                            <h3 class='ladi-headline'>Khách hàng quay lại</h3>
                        </div>
                        <div id="SHAPE61" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1536 1896.0833" fill="rgba(232, 105, 48, 1.0)"> <path d="M1134 1075q-37 121-138 195t-228 74-228-74-138-195q-8-25 4-48.5t38-31.5q25-8 48.5 4t31.5 38q25 80 92.5 129.5T768 1216t151.5-49.5T1012 1037q8-26 32-38t49-4 37 31.5 4 48.5zM640 640q0 53-37.5 90.5T512 768t-90.5-37.5T384 640t37.5-90.5T512 512t90.5 37.5T640 640zm512 0q0 53-37.5 90.5T1024 768t-90.5-37.5T896 640t37.5-90.5T1024 512t90.5 37.5T1152 640zm256 256q0-130-51-248.5t-136.5-204-204-136.5T768 256t-248.5 51-204 136.5-136.5 204T128 896t51 248.5 136.5 204 204 136.5 248.5 51 248.5-51 204-136.5 136.5-204 51-248.5zm128 0q0 209-103 385.5T1153.5 1561 768 1664t-385.5-103T103 1281.5 0 896t103-385.5T382.5 231 768 128t385.5 103T1433 510.5 1536 896z"></path> </svg></div>
                        </div>
                    </div>
                </div>
                <div id="GROUP304" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE53" class="ladi-element">
                            <h3 class='ladi-headline'>8.5K</h3>
                        </div>
                        <div id="HEADLINE57" class="ladi-element">
                            <h3 class='ladi-headline'>Dùng thường xuyên</h3>
                        </div>
                        <div id="SHAPE62" class="ladi-element">
                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 1664 1896.0833" fill="rgba(232, 105, 48, 1.0)"> <path d="M1664 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5T1321 1619q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5T301 1569q0-6 2-20l86-500L25 695Q0 668 0 647q0-37 56-46l502-73L783 73q19-41 49-41t49 41l225 455 502 73q56 9 56 46z"></path> </svg></div>
                        </div>
                    </div>
                </div>
                <div id="GROUP300" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE54" class="ladi-element">
                            <h3 class='ladi-headline'>NHỮNG CON SỐ KHÔNG BIẾT NÓI DỐI</h3>
                        </div>
                        <div id="GROUP270" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="LINE267" class="ladi-element">
                                    <div class='ladi-line'>
                                        <div class="ladi-line-container"></div>
                                    </div>
                                </div>
                                <div id="SHAPE268" class="ladi-element">
                                    <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 324.08 107.96" fill="rgba(232, 105, 48, 1.0)"><path d="M270.09,0l54,54-54,54-54-54Zm39,54-39-39-39,39,39,39Z"></path><path d="M162.13,0l54,54-54,54-54-54Zm39,54-39-39-39,39,39,39Z"></path><path d="M54,0l54,54L54,108,0,54ZM93,54,54,15,15,54,54,93Z"></path></svg></div>
                                </div>
                                <div id="LINE269" class="ladi-element">
                                    <div class='ladi-line'>
                                        <div class="ladi-line-container"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION275" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP294" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="FORM276" data-config-id="5e9809e2b8717c5ad157b3f9" class="ladi-element">
                            <form autocomplete="off" method="post" class='ladi-form'>
                                <div id="FORM_ITEM281" class="ladi-element">
                                    <div class="ladi-form-item-container">
                                        <div class="ladi-form-item-background" style="background: rgb(240, 238, 175) !important"></div>
                                        <div class='ladi-form-item'>
                                            <div class="thrv_wrapper tve_image_caption" data-css="tve-u-173e8951a51" style="">
                                                <span class="tve_image_frame">
                                                    <img class="tve_image wp-image-121" alt="ky thuat" data-id="121" width="56" data-init-width="1280" height="56" data-init-height="1280" title="ky thuat" loading="lazy" src="/img/avt_long.jpg" data-width="56" data-height="56" data-css="tve-u-173e8953c69" style="" sizes="(max-width: 56px) 100vw, 56px">
                                                </span>
                                            </div>
                                            <div class="ladi-group">
                                                <p data-css="tve-u-1747221ba34" style="margin-left: 91px; margin-top: -39px">
                                                    <strong style="font-weight: bold"><span data-css="tve-u-1747221ba7a" style="color: rgb(232, 105, 48);">Lê Văn Long</span></strong>
                                                </p>
                                                <p data-css="tve-u-173e8aa8ab2" style="margin-left: 91px; margin-top: 0px">
                                                    <span data-css="tve-u-1747221bac0" style="color: rgb(232, 105, 48);">Hỗ trợ tư vấn</span>
                                                </p>
                                                <div class="" style="margin-top: 20px;display: flex;">
                                                    <div style="width: 177px;background: rgb(32, 183, 42); border-radius: 8px;margin-left: 70px;">
                                                        <a href="tel:0978093540" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 24 24" data-id="icon-phone-regular" data-name="">
                                                                        <path d="M6.62 10.79a15.15 15.15 0 0 0 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"></path>
                                                                    </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 54px;margin-top: -27px;">0978093540</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                    <div style="width: 177px;background: rgb(7, 150, 255) !important; border-radius: 8px;margin-left: 7px;">
                                                        <a href="https://zalo.me/0978093540" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 512 512" data-id="icon-link-solid" data-name="">
                                                                        <path d="M326.612 185.391c59.747 59.809 58.927 155.698.36 214.59-.11.12-.24.25-.36.37l-67.2 67.2c-59.27 59.27-155.699 59.262-214.96 0-59.27-59.26-59.27-155.7 0-214.96l37.106-37.106c9.84-9.84 26.786-3.3 27.294 10.606.648 17.722 3.826 35.527 9.69 52.721 1.986 5.822.567 12.262-3.783 16.612l-13.087 13.087c-28.026 28.026-28.905 73.66-1.155 101.96 28.024 28.579 74.086 28.749 102.325.51l67.2-67.19c28.191-28.191 28.073-73.757 0-101.83-3.701-3.694-7.429-6.564-10.341-8.569a16.037 16.037 0 0 1-6.947-12.606c-.396-10.567 3.348-21.456 11.698-29.806l21.054-21.055c5.521-5.521 14.182-6.199 20.584-1.731a152.482 152.482 0 0 1 20.522 17.197zM467.547 44.449c-59.261-59.262-155.69-59.27-214.96 0l-67.2 67.2c-.12.12-.25.25-.36.37-58.566 58.892-59.387 154.781.36 214.59a152.454 152.454 0 0 0 20.521 17.196c6.402 4.468 15.064 3.789 20.584-1.731l21.054-21.055c8.35-8.35 12.094-19.239 11.698-29.806a16.037 16.037 0 0 0-6.947-12.606c-2.912-2.005-6.64-4.875-10.341-8.569-28.073-28.073-28.191-73.639 0-101.83l67.2-67.19c28.239-28.239 74.3-28.069 102.325.51 27.75 28.3 26.872 73.934-1.155 101.96l-13.087 13.087c-4.35 4.35-5.769 10.79-3.783 16.612 5.864 17.194 9.042 34.999 9.69 52.721.509 13.906 17.454 20.446 27.294 10.606l37.106-37.106c59.271-59.259 59.271-155.699.001-214.959z"></path>
                                                                      </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 64px; margin-top: -25px;">Zalo</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div id="FORM_ITEM1997" class="ladi-element">
                                    <div class="ladi-form-item-container">
                                        <div class="ladi-form-item-background" style="background: rgb(240, 238, 175) !important"></div>
                                        <div class='ladi-form-item'>
                                            <div class="thrv_wrapper tve_image_caption" data-css="tve-u-173e8951a51" style="">
                                                <span class="tve_image_frame">
                                                    <img class="tve_image wp-image-121" alt="ky thuat" data-id="121" width="56" data-init-width="1280" height="56" data-init-height="1280" title="ky thuat" loading="lazy" src="/img/avt_long.jpg" data-width="56" data-height="56" data-css="tve-u-173e8953c69" style="" sizes="(max-width: 56px) 100vw, 56px">
                                                </span>
                                            </div>
                                            <div class="ladi-group">
                                                <p data-css="tve-u-1747221ba34" style="margin-left: 91px; margin-top: -39px">
                                                    <strong style="font-weight: bold"><span data-css="tve-u-1747221ba7a" style="color: rgb(232, 105, 48);">Lê Văn Long</span></strong>
                                                </p>
                                                <p data-css="tve-u-173e8aa8ab2" style="margin-left: 91px; margin-top: 0px">
                                                    <span data-css="tve-u-1747221bac0" style="color: rgb(232, 105, 48);">Hỗ trợ kĩ thuật</span>
                                                </p>
                                                <div class="" style="margin-top: 20px;display: flex;">
                                                    <div style="width: 177px;background: rgb(32, 183, 42); border-radius: 8px;margin-left: 70px;">
                                                        <a href="tel:0978093540" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 24 24" data-id="icon-phone-regular" data-name="">
                                                                        <path d="M6.62 10.79a15.15 15.15 0 0 0 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"></path>
                                                                    </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 54px;margin-top: -27px;">0978093540</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                    <div style="width: 177px;background: rgb(7, 150, 255) !important; border-radius: 8px;margin-left: 7px;">
                                                        <a href="https://zalo.me/0978093540" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 512 512" data-id="icon-link-solid" data-name="">
                                                                        <path d="M326.612 185.391c59.747 59.809 58.927 155.698.36 214.59-.11.12-.24.25-.36.37l-67.2 67.2c-59.27 59.27-155.699 59.262-214.96 0-59.27-59.26-59.27-155.7 0-214.96l37.106-37.106c9.84-9.84 26.786-3.3 27.294 10.606.648 17.722 3.826 35.527 9.69 52.721 1.986 5.822.567 12.262-3.783 16.612l-13.087 13.087c-28.026 28.026-28.905 73.66-1.155 101.96 28.024 28.579 74.086 28.749 102.325.51l67.2-67.19c28.191-28.191 28.073-73.757 0-101.83-3.701-3.694-7.429-6.564-10.341-8.569a16.037 16.037 0 0 1-6.947-12.606c-.396-10.567 3.348-21.456 11.698-29.806l21.054-21.055c5.521-5.521 14.182-6.199 20.584-1.731a152.482 152.482 0 0 1 20.522 17.197zM467.547 44.449c-59.261-59.262-155.69-59.27-214.96 0l-67.2 67.2c-.12.12-.25.25-.36.37-58.566 58.892-59.387 154.781.36 214.59a152.454 152.454 0 0 0 20.521 17.196c6.402 4.468 15.064 3.789 20.584-1.731l21.054-21.055c8.35-8.35 12.094-19.239 11.698-29.806a16.037 16.037 0 0 0-6.947-12.606c-2.912-2.005-6.64-4.875-10.341-8.569-28.073-28.073-28.191-73.639 0-101.83l67.2-67.19c28.239-28.239 74.3-28.069 102.325.51 27.75 28.3 26.872 73.934-1.155 101.96l-13.087 13.087c-4.35 4.35-5.769 10.79-3.783 16.612 5.864 17.194 9.042 34.999 9.69 52.721.509 13.906 17.454 20.446 27.294 10.606l37.106-37.106c59.271-59.259 59.271-155.699.001-214.959z"></path>
                                                                      </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 64px; margin-top: -25px;">Zalo</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div id="FORM_ITEM2002" class="ladi-element">
                                    <div class="ladi-form-item-container">
                                        <div class="ladi-form-item-background" style="background: rgb(240, 238, 175) !important"></div>
                                        <div class='ladi-form-item'>
                                            <div class="thrv_wrapper tve_image_caption" data-css="tve-u-173e8951a51" style="">
                                                <span class="tve_image_frame">
                                                    <img class="tve_image wp-image-121" alt="ky thuat" data-id="121" width="56" data-init-width="1280" height="56" data-init-height="1280" title="ky thuat" loading="lazy" src="/img/avt_nga.jpg" style="object-fit: cover" data-width="56" data-height="56" data-css="tve-u-173e8953c69" style="" sizes="(max-width: 56px) 100vw, 56px">
                                                </span>
                                            </div>
                                            <div class="ladi-group">
                                                <p data-css="tve-u-1747221ba34" style="margin-left: 91px; margin-top: -39px">
                                                    <strong style="font-weight: bold"><span data-css="tve-u-1747221ba7a" style="color: rgb(232, 105, 48);">Trần Thị Thúy Nga</span></strong>
                                                </p>
                                                <p data-css="tve-u-173e8aa8ab2" style="margin-left: 91px; margin-top: 0px">
                                                    <span data-css="tve-u-1747221bac0" style="color: rgb(232, 105, 48);">Hỗ trợ tư vấn</span>
                                                </p>
                                                <div class="" style="margin-top: 20px;display: flex;">
                                                    <div style="width: 177px;background: rgb(32, 183, 42); border-radius: 8px;margin-left: 70px;">
                                                        <a href="tel:0978093540" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 24 24" data-id="icon-phone-regular" data-name="">
                                                                        <path d="M6.62 10.79a15.15 15.15 0 0 0 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"></path>
                                                                    </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 54px;margin-top: -27px;">0332429715</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                    <div style="width: 177px;background: rgb(7, 150, 255) !important; border-radius: 8px;margin-left: 7px;">
                                                        <a href="https://zalo.me/0332429715" class="tcb-button-link tcb-plain-text" style="letter-spacing: var(--tve-letter-spacing, 2px);">
                                                            <span class="tcb-button-icon">
                                                                <div class="thrv_icon tve_no_drag tve_no_icons tcb-icon-inherit-style tcb-icon-display" data-css="tve-u-173e8a39feb">
                                                                    <svg class="tcb-icon" viewBox="0 0 512 512" data-id="icon-link-solid" data-name="">
                                                                        <path d="M326.612 185.391c59.747 59.809 58.927 155.698.36 214.59-.11.12-.24.25-.36.37l-67.2 67.2c-59.27 59.27-155.699 59.262-214.96 0-59.27-59.26-59.27-155.7 0-214.96l37.106-37.106c9.84-9.84 26.786-3.3 27.294 10.606.648 17.722 3.826 35.527 9.69 52.721 1.986 5.822.567 12.262-3.783 16.612l-13.087 13.087c-28.026 28.026-28.905 73.66-1.155 101.96 28.024 28.579 74.086 28.749 102.325.51l67.2-67.19c28.191-28.191 28.073-73.757 0-101.83-3.701-3.694-7.429-6.564-10.341-8.569a16.037 16.037 0 0 1-6.947-12.606c-.396-10.567 3.348-21.456 11.698-29.806l21.054-21.055c5.521-5.521 14.182-6.199 20.584-1.731a152.482 152.482 0 0 1 20.522 17.197zM467.547 44.449c-59.261-59.262-155.69-59.27-214.96 0l-67.2 67.2c-.12.12-.25.25-.36.37-58.566 58.892-59.387 154.781.36 214.59a152.454 152.454 0 0 0 20.521 17.196c6.402 4.468 15.064 3.789 20.584-1.731l21.054-21.055c8.35-8.35 12.094-19.239 11.698-29.806a16.037 16.037 0 0 0-6.947-12.606c-2.912-2.005-6.64-4.875-10.341-8.569-28.073-28.073-28.191-73.639 0-101.83l67.2-67.19c28.239-28.239 74.3-28.069 102.325.51 27.75 28.3 26.872 73.934-1.155 101.96l-13.087 13.087c-4.35 4.35-5.769 10.79-3.783 16.612 5.864 17.194 9.042 34.999 9.69 52.721.509 13.906 17.454 20.446 27.294 10.606l37.106-37.106c59.271-59.259 59.271-155.699.001-214.959z"></path>
                                                                      </svg>
                                                                </div>
                                                            </span>
                                                            <span class="tcb-button-texts">
                                                                <span class="tcb-button-text thrv-inline-text" data-css="tve-u-173e89e6055" style="margin-left: 64px; margin-top: -25px;">Zalo</span>
                                                                <span class="tcb-secondary-text thrv-inline-text"></span>
                                                            </span>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        </div>
                        <div id="HEADLINE285" class="ladi-element">
                            <h3 class='ladi-headline'>Liên hệ với chúng tôi , chỉ 3 phút DỊCH VỤ shopee  sẽ điện lại và tư vấn những thắc mắc cho quý khách</h3>
                        </div>
                        <div id="GROUP293" class="ladi-element">
                            <div class='ladi-group'>
                                <div id="GROUP290" class="ladi-element">
                                    <div class='ladi-group'>
                                        <div id="HEADLINE282" class="ladi-element">
                                            <h3 class='ladi-headline'>ĐĂNG KÝ TƯ VẤN</h3>
                                        </div>
                                        <div id="SHAPE288" class="ladi-element">
                                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 218.3 106.67" fill="rgba(232, 105, 48, 1.0)"><polygon style="opacity:0.3;" points="218.3 53.34 125.91 0 125.91 106.67 218.3 53.34"></polygon><polygon style="opacity:0.6;" points="155.34 53.34 62.96 0 62.96 106.67 155.34 53.34"></polygon><polygon points="92.38 53.34 0 0 0 106.67 92.38 53.34"></polygon></svg></div>
                                        </div>
                                        <div id="SHAPE289" class="ladi-element">
                                            <div class='ladi-shape'><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 218.3 106.67" fill="rgba(232, 105, 48, 1.0)"><polygon style="opacity:0.3;" points="218.3 53.34 125.91 0 125.91 106.67 218.3 53.34"></polygon><polygon style="opacity:0.6;" points="155.34 53.34 62.96 0 62.96 106.67 155.34 53.34"></polygon><polygon points="92.38 53.34 0 0 0 106.67 92.38 53.34"></polygon></svg></div>
                                        </div>
                                    </div>
                                </div>
                                <div id="LINE291" class="ladi-element">
                                    <div class='ladi-line'>
                                        <div class="ladi-line-container"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION63" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="GROUP254" class="ladi-element">
                    <div class='ladi-group'>
                        <div id="HEADLINE64" class="ladi-element">
                            <h1 class='ladi-headline'>Dịch vụ shopee&nbsp;</h1>
                        </div>
                        <div id="HEADLINE65" class="ladi-element">
                            <h3 class='ladi-headline'>LIÊN HỆ CHÚNG TÔI</h3>
                        </div>
                        <div id="HEADLINE66" class="ladi-element">
                            <h3 class='ladi-headline'>Địa chỉ : Thành Đỏ - Phúc Yên - Vĩnh Phúc&nbsp;</h3>
                        </div>
                        <div id="HEADLINE67" class="ladi-element">
                            <h3 class='ladi-headline'>Hotline : 0978093540</h3>
                        </div>
                        <div id="HEADLINE68" class="ladi-element">
                            <h3 class='ladi-headline'>Email : dichvushopee@gmail.com</h3>
                        </div>
                        <div id="HEADLINE69" class="ladi-element">
                            <h3 class='ladi-headline'>THEO DÕI CHÚNG TÔI TRÊN MẠNG XÃ HỘI</h3>
                        </div>
                        <div id="HEADLINE70" class="ladi-element">
                            <h3 class='ladi-headline'>Theo dõi fanpage của chúng tôi để theo dõi những bài viết mới và hỗ trợ nhau bán hàng</h3>
                        </div>
                        <div id="HEADLINE71" class="ladi-element">
                            <h3 class='ladi-headline'>Fanpage : https://www.facebook.com/C.I.O.Support.Shopee</h3>
                        </div>
                        <div id="HEADLINE251" class="ladi-element">
                            <h3 class='ladi-headline'>UY TÍN - CHUYÊN NGHIỆP - AN TOÀN</h3>
                        </div>
                        <div id="LINE252" class="ladi-element">
                            <div class='ladi-line'>
                                <div class="ladi-line-container"></div>
                            </div>
                        </div>
                        <div id="LINE253" class="ladi-element">
                            <div class='ladi-line'>
                                <div class="ladi-line-container"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="IMAGE255" class="ladi-element">
                    <div class='ladi-image'>
                        <div class="ladi-image-background"></div>
                    </div>
                </div>
                <div id="NOTIFY306" class="ladi-element">
                    <div class="ladi-notify ladi-hidden">
                        <div class="ladi-notify-image"><img src="source/notify.svg" /></div>
                        <div class="ladi-notify-title">Nội dung cột [Title]</div>
                        <div class="ladi-notify-content">Nội dung cột [Content]</div>
                        <div class="ladi-notify-time">Nội dung cột [Time]</div>
                    </div>
                </div>
            </div>
        </div>
        <div id="SECTION_POPUP" class="ladi-section">
            <div class='ladi-section-background'></div>
            <div class="ladi-container">
                <div id="POPUP203" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE204" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP205" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE206" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP207" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE208" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP209" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE211" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP212" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE213" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP214" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE215" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="POPUP216" class="ladi-element">
                    <div class='ladi-popup'>
                        <div class="ladi-overlay"></div>
                        <div id="IMAGE217" class="ladi-element">
                            <div class='ladi-image'>
                                <div class="ladi-image-background"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
    <div id="backdrop-popup" class="backdrop-popup"></div>
    <div id="lightbox-screen" class="lightbox-screen"></div>
    <link href="https://fonts.googleapis.com/css?family=Open Sans:bold,regular|Dancing Script:bold,regular&display=swap" rel="stylesheet" type="text/css">
    <link href="{{ asset('v2/source/ladipage.min.css?v=1588161697899') }}" rel="stylesheet" type="text/css">

    @include('frontend.js')
    <!-- Load Facebook SDK for JavaScript -->
    <div id="fb-root"></div>
    <script>
        window.fbAsyncInit = function() {
            FB.init({
            xfbml            : true,
            version          : 'v9.0'
            });
        };

        (function(d, s, id) {
        var js, fjs = d.getElementsByTagName(s)[0];
            if (d.getElementById(id)) return;
            js = d.createElement(s); js.id = id;
            js.src = 'https://connect.facebook.net/vi_VN/sdk/xfbml.customerchat.js';
            fjs.parentNode.insertBefore(js, fjs);
        }(document, 'script', 'facebook-jssdk'));
    </script>

        <!-- Your Chat Plugin code -->
    <div class="fb-customerchat"
        attribution=setup_tool
        page_id="185077411631892"
        logged_in_greeting="Chào mừng bạn đến với dichvushopee. Chúng tôi có thể giúp gì cho bạn ?"
        logged_out_greeting="Chào mừng bạn đến với dichvushopee. Chúng tôi có thể giúp gì cho bạn ?">
    </div>
</body>

</html>